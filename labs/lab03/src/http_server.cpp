...
//Ініціюємо бібліотеку сокетів
    if (initSocketAPI()){
        socketError(TRUE,(char*)"init socket API");
        return -1;
    }
...
//Зв’язуємо сокет з адресою машини
    if (bind(listenSocket, (struct sockaddr *)&serverAddr,
             sizeof(serverAddr))){
        socketError(TRUE,(char*)"bind socket");
        return -2;
    }
    listen(listenSocket, LINEQ);
    printInfo(argv[0],(char*)DESCRIPTION);
...
//роботи серверу.
    for (unsigned long long socket : sockets){
        if (socket>0)
            send(socket,shutdownServerCmd,
